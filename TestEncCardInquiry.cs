namespace Moneris
{
	...
	public class TestEncCardInquiry
	{
	  public static void Main(string[] args)
	  {
            ...
            string language_code = args[2];
            string cvd_value = args[3];

            EncErnexGiftCardInq ernexGiftCardInq = new EncErnexGiftCardInq (track2, language_code);

            ernexGiftCardInq.SetCvdValue(cvd_value);
            
            ErnexHttpsPostRequest mpgReq =
                new ErnexHttpsPostRequest(host, store_id, api_token, ernexGiftCardInq);
